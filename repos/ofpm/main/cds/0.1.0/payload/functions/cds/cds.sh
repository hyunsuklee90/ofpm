...
_cds_migrate() {
    local db_file="$CDS_DATA_DIR/cds_db"
    local old_path="$CDS_DATA_DIR/cds_path"
    local old_tag="$CDS_DATA_DIR/cds_tag"
    local old_idx="$CDS_DATA_DIR/cds_idx"
    ...
    if [[ ! -f "$db_file" ]]; then
        for profile_dir in "$CDS_ROOT"/legacy/config/*; do
...
_cds_get_active_db() {
    ...
    if [[ "$mode" == "memory" && -f "$_CDS_SESSION_DB" ]]; then
        ...
        echo "$CDS_DATA_DIR/cds_db"
    fi
}

_cds_backup() {
    local active_db=$(_cds_get_active_db)
    local backup_dir="$CDS_DATA_DIR/backups"
    [[ ! -f "$active_db" ]] && return
...
_cds_log() {
    local path="$1"
    local history_file="$CDS_DATA_DIR/cds_history"
...
cds() {
    ...
    case "$cmd" in
        ...
        "-z")
            local query="$2"
            if [[ -z "$query" ]]; then
                sort "$CDS_DATA_DIR/cds_history" | uniq -c | sort -nr | head -n 10 >&2
            else
